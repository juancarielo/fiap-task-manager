...
const saveTask = async (req: NextApiRequest, res: NextApiResponse<DefaultResponse>, userId: string) => {
    if (req.body) {
...
const getTasks = async (req: NextApiRequest, res: NextApiResponse<DefaultResponse | Task[]>, userId: string) => {
...
const updateTask = async (req: NextApiRequest, res: NextApiResponse<DefaultResponse | Task[]>, userId: string) => {
...
const deleteTask = async (req: NextApiRequest, res: NextApiResponse<DefaultResponse | Task[]>, userId: string) => {
    const taskFound = await validateTaskAndReturnValue(req, userId);
    if (!taskFound) {
        return res.status(400).json({ error: "Tarefa não encontrada." });
    }
